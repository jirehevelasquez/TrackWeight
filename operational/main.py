...
# BOOT button can also stop session while reps are being counted
...
SUPABASE_KEY = "SUPA KEY" # Private information" update accordingly
...
def sync_time():
    try:
        ntptime.settime()  # Syncs ESP32 RTC to UTC using NTP
...
def send_workout_to_supabase(weight_lbs, reps):
    ...
    Current Supabase payload sends:
        machine_name
        reps
        user_slot

    weight_lbs is commented out for now because the Supabase table
    currently does not have a weight_lbs column.
    """
    ...
    payload = {
        ...
        # Uncomment this after adding a weight_lbs column in Supabase.
        "weight": weight_lbs,
        "created_at" : get_timestamp_utc()
    }
...
def should_stop_session():
    """
    During rep counting, pressing BOOT again ends the current workout session.
    """
...
def count_reps():
    ...
    print("Press BOOT again to stop the workout manually.")
